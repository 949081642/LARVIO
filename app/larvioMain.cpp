...
int main(int argc, char** argv) {
  if (argc != 5) {
    cerr << endl
         << "Usage: ./larvio path_to_imu/data.csv path_to_cam0/data.csv "
            "path_to_cam0/data config_file_path"
         << endl;
    return 1;
  }
  ...
  glEnable(
      GL_DEPTH_TEST);  // 3D Mouse handler requires depth testing to be enabled
  ...
  pangolin::Var<bool> menuShowActivePoints("menu.Show Active Points", true,
                                           true);
  pangolin::Var<bool> menuShowStablePoints("menu.Show Stable Points", true,
                                           true);
  pangolin::Var<bool> menuShowSlideWindow("menu.Show Slide Window", true, true);
  pangolin::OpenGlRenderState
      s_cam(  // Define Camera Render Object (for view / scene browsing)
          pangolin::ProjectionMatrix(1024, 768, 500, 500, 512, 389, 0.1, 1000),
          pangolin::ModelViewLookAt(0, 3.5, 9, 0, 0, 0, 0, -1, 0));
  pangolin::View& d_cam =
      pangolin::CreateDisplay()  // Add named OpenGL viewport to window and
                                 // provide 3D Handler
          .SetBounds(0.0, 1.0, pangolin::Attach::Pix(175), 1.0,
                     -1024.0f / 768.0f)
          .SetHandler(new pangolin::Handler3D(s_cam));
  pangolin::View& d_image =
      pangolin::CreateDisplay()  // Add named OpenGL viewport to image and
                                 // provide 3D Handler
          .SetBounds(pangolin::Attach::Pix(0), 1.0f, pangolin::Attach::Pix(175),
                     1 / 3.5f, 752.0 / 480)
  ...
  for (size_t j = 0; j < allImgInfo.size(); j++) {
    // get img
    string temp =
        allImgInfo[j].imgName.substr(0, allImgInfo[j].imgName.size() - 1);
    ...
    // get imus
    while (allImuData[k].timeStampToSec - imgPtr->timeStampToSec < 0.05 &&
           k < allImuData.size()) {
      imu_msg_buffer.push_back(larvio::ImuData(
          allImuData[k].timeStampToSec, allImuData[k].angular_velocity,
          allImuData[k].linear_acceleration));
      ++k;
    }

    // process
    larvio::MonoCameraMeasurementPtr features =
        new larvio::MonoCameraMeasurement;
    int64 start_time_fe = cv::getTickCount();
    bool bProcess = ImgProcesser->processImage(imgPtr, imu_msg_buffer,
                                               features);  // process image
    int64 end_time_fe = cv::getTickCount();
    double processing_time_fe =
        (end_time_fe - start_time_fe) / cv::getTickFrequency();
    double processing_time_be;
    bool bPubOdo = false;
    if (bProcess) {
      int64 start_time_be = cv::getTickCount();
      bPubOdo = Estimator->processFeatures(features,
                                           imu_msg_buffer);  // state estimation
      int64 end_time_be = cv::getTickCount();
      processing_time_be =
          (end_time_be - start_time_be) / cv::getTickFrequency();
    }
    if (bPubOdo) {  // Visualization
      // draw 3D
      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
      larvio::visualize::GetCurrentOpenGLPoseMatrix(Tbw_pgl,
                                                    Estimator->getTbw());
