...
class FlexibleInitializer {
 public:
  ...
  FlexibleInitializer(const double& max_feature_dis_, const int& static_Num_,
    ...
    dynamicInitPtr.reset(new DynamicInitializer(td_, Ma_, Tg_, As_, acc_n_,
                                                acc_w_, gyr_n_, gyr_w_, R_c2b,
                                                t_bc_b, imu_img_timeTh_));
  }
  ...
  bool tryIncInit(std::vector<ImuData>& imu_msg_buffer,
                  MonoCameraMeasurementPtr img_msg, Eigen::Vector3d& m_gyro_old,
