...
class LarVio {
 public:
  ...
  void getStableMapPointPositions(
      std::map<larvio::FeatureIDType, Eigen::Vector3d>& mMapPoints);
  void getActiveeMapPointPositions(
      std::map<larvio::FeatureIDType, Eigen::Vector3d>& mMapPoints);
 ...
 private:
  ...
  struct StateServer {
    ...
    // timestamp compensation: t_imu = t_cam + td, timestamp of camera has to
    // add @td to synchronize with clock of imu
    double td;
  ...
  void findRedundantImuStates(std::vector<StateIDType>& rm_state_ids);
  ...
  // store the distances of matched feature coordinates of current and previous
  // image, for ZUPT(zero velocity update), added by QXC
  ...
  void measurementUpdate_ZUPT_vpq();  // measurement function for ZUPT, with
                                      // measurement of velocity, relative
                                      // position and relative quaterinion
  ...
  Eigen::Vector3d
      meanSForce;  // store the mean of accelerator between two frames
  ...
  float Static_Duration;  // static scene duration for inclinometer-initializer
                          // to utlize, in seconds.
  int Static_Num;  // static img mumber for inclinometer-initializer to utlize,
                   // calculate according to @Static_Duration and @features_rate
  ...
  void measurementUpdate_hybrid(const Eigen::MatrixXd& H_ekf_new,
  ...
  // For feature points distribution, got to know the camera instrinc and image
  // resolution
  ...
  static bool compareByObsNum(const std::pair<FeatureIDType, int>& lhs,
                              const std::pair<FeatureIDType, int>& rhs) {
  ...
  StateIDType getNewAnchorId(Feature& feature,
