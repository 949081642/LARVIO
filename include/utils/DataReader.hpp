/*
 * @Descripttion: This header include functions and types for reading IMU and
 * image data, and methods to manipulate.
 * @Author: Xiaochen Qiu
 */
...
void loadImuFile(char *imuPath, vector<ImuData> &vimuData) {
  ...
  while (!inf.eof()) {
    ...
    ImuData tempImu(imuTimeStamp, grad[0], grad[1], grad[2], acc[0], acc[1],
                    acc[2]);
...
bool findFirstAlign(const vector<ImuData> &vImu, const vector<ImgInfo> &vImg,
                    pair<int, int> &ImgImuAlign) {
