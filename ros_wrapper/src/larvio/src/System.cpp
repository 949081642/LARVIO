...
bool System::createRosIO() {
  ...
  stable_feature_pub =
      nh.advertise<sensor_msgs::PointCloud2>("stable_feature_point_cloud", 1);
  active_feature_pub =
      nh.advertise<sensor_msgs::PointCloud2>("active_feature_point_cloud", 1);
...
void System::imuCallback(const sensor_msgs::ImuConstPtr& msg) {
  imu_msg_buffer.push_back(
      ImuData(msg->header.stamp.toSec(), msg->angular_velocity.x,
...
void System::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  if (imu_msg_buffer.empty()) return;

  // test
  cv_bridge::CvImageConstPtr cvCPtr =
  ...
  if (!imu_msg_buffer.empty() ||
      (imu_msg_buffer.end() - 1)->timeStampToSec - msgPtr->timeStampToSec <
          -imu_img_timeTh) {
    img_msg_buffer.push_back(msgPtr);
    header_buffer.push_back(header);
  }
  if (!img_msg_buffer.empty()) {
    if ((imu_msg_buffer.end() - 1)->timeStampToSec -
            (*(img_msg_buffer.begin()))->timeStampToSec <
        -imu_img_timeTh)
      return;
    bUseBuff = true;
  }
  ...
  if (!bUseBuff) {
    ...
    bool bProcess =
        ImgProcesser->processImage(msgPtr, imu_msg_buffer, features);
    ...
    for (int i = 0; i < img_msg_buffer.size(); ++i) {
      ...
      if ((imu_msg_buffer.end() - 1)->timeStampToSec -
              img_msg_buffer[i]->timeStampToSec <
          -imu_img_timeTh)
        break;
      ...
      // Process image to get feature measurement.
      bool bProcess = ImgProcesser->processImage(img_msg_buffer[i],
                                                 imu_msg_buffer, features);
      ...
      // Publish msgs if necessary
      if (bProcess) {
        cv_bridge::CvImage _image(header_buffer[i], "bgr8",
                                  ImgProcesser->getVisualImg());
        vis_img_pub.publish(_image.toImageMsg());
      }
      if (bPubOdo) {
    ...
    img_msg_buffer.erase(img_msg_buffer.begin(),
                         img_msg_buffer.begin() + counter);
    header_buffer.erase(header_buffer.begin(), header_buffer.begin() + counter);
  }
}

// Publish informations of VIO, including odometry, path, points cloud and
// whatever needed.
