...
ORBdescriptor::ORBdescriptor(const cv::Mat& _image, float _scaleFactor,
                             int _nlevels, int _edgeThreshold, int _patchSize)
...
#define GET_VALUE(idx)                                             \
  (x = pattern_[idx].x * a - pattern_[idx].y * b,                  \
   y = pattern_[idx].x * b + pattern_[idx].y * a, ix = cvRound(x), \
   iy = cvRound(y), *(center + iy * step + ix))
...
bool ORBdescriptor::computeDescriptors(const vector<cv::Point2f>& pts,
                                       ...
                                       cv::Mat& descriptors) {
  // return false if @mvLayerInfo, @mImagePyramid and @mBluredImagePyramid
  // haven't been initialized
  if (0 == mvLayerInfo.size() || 0 == mvLayerScale.size() ||
      mImagePyramid.empty() || mBluredImagePyramid.empty()) {
    cout << "In ORBdescriptor::computeDescriptors(): Should have call "
            "ORBdescriptor::initializeLayerAndPyramid before calling this !"
         << endl;
    return false;
  }
...
void ORBdescriptor::initializeLayerAndPyramid(const cv::Mat& image) {
  ...
  cv::Size bufSize(
      (cvRound(image.cols * mvInvScaleFactor[0]) + border * 2 + 15) & -16, 0);
  ...
  for (int level = 0; level < nlevels; level++) {
    ...
    cv::Rect linfo(level_ofs.x + border, level_ofs.y + border, sz.width,
                   sz.height);
  ...
  if (!mImagePyramid.empty()) mImagePyramid.release();
  ...
  for (int level = 0; level < nlevels; ++level) {
    ...
    cv::Size wholeSize(sz.width + border * 2, sz.height + border * 2);
    cv::Rect wholeLinfo = cv::Rect(linfo.x - border, linfo.y - border,
...
float ORBdescriptor::IC_Angle(const int& level, const cv::Point2f& pt) {
  ...
  const uchar* center = &mImagePyramid.at<uchar>(cvRound(pt_.y) + layer.y,
                                                 cvRound(pt_.x) + layer.x);

  int m_01 = 0, m_10 = 0;

  // Treat the center line differently, v=0
  for (int u = -halfPatchSize; u <= halfPatchSize; ++u) m_10 += u * center[u];
...
void ORBdescriptor::makeRandomPattern(int patchSize, cv::Point* pattern_,
                                      int npoints) {
