...
bool StaticInitializer::tryIncInit(const std::vector<ImuData>& imu_msg_buffer,
  ...
  if (maxDis < max_feature_dis) {
    ...
    if (staticImgCounter <
        static_Num)  // return false if number of consecitive static images does
                     // not reach @static_Num
      return false;
  } else {
...
void StaticInitializer::initializeGravityAndBias(
  ...
  for (const auto& imu_msg : imu_msg_buffer) {
    ...
    sum_angular_vel +=
        Tg * (imu_msg.angular_velocity - As * Ma * imu_msg.linear_acceleration);
...
void StaticInitializer::assignInitialState(std::vector<ImuData>& imu_msg_buffer,
  ...
  if (usefulImuSize >= imu_msg_buffer.size()) usefulImuSize--;
