...
bool ImageProcessor::loadParameters() {
  ...
  processor_config.flag_equalize =
      (static_cast<int>(fsSettings["flag_equalize"]) ? true : false);
  ...
  // cout << ".ransac_threshold = " << processor_config.ransac_threshold <<
  // endl; cout << ".max_features_num = " << processor_config.max_features_num
  // << endl; cout << ".min_distance = " << processor_config.min_distance <<
  // endl; cout << ".flag_equalize = " << processor_config.flag_equalize <<
  // endl; cout << ".pub_frequency = " << processor_config.pub_frequency <<
  // endl; cout << "cam_distortion_model = " << cam_distortion_model << endl;
...
bool ImageProcessor::processImage(const ImageDataPtr& msg,
  ...
  currORBDescriptor_ptr.reset(
      new ORBdescriptor(curr_pyramid_[0], 2, processor_config.pyramid_levels));
  ...
  if (FIRST_IMAGE == image_state) {
    if (initializeFirstFrame()) image_state = SECOND_IMAGE;
  } else if (SECOND_IMAGE == image_state) {
    if (!initializeFirstFeatures(imu_msg_buffer)) {
      image_state = FIRST_IMAGE;
    } else {
      // frequency control
      if (curr_img_time - last_pub_time >=
    ...
    // Integrate gyro data to get a guess of rotation between current and
    // previous image
...
void ImageProcessor::integrateImuData(
    Matx33f& cam_R_p2c, const std::vector<ImuData>& imu_msg_buffer) {
  // Find the start and the end limit within the imu msg buffer.
  auto begin_iter = imu_msg_buffer.begin();
  while (begin_iter != imu_msg_buffer.end()) {
    if (begin_iter->timeStampToSec - prev_img_ptr->timeStampToSec < -0.0049)
      ++begin_iter;
    else
      break;
  }
  ...
  while (end_iter != imu_msg_buffer.end()) {
    if (end_iter->timeStampToSec - curr_img_ptr->timeStampToSec < 0.0049)
      ++end_iter;
    else
      break;
  }
  ...
  for (auto iter = begin_iter; iter < end_iter; ++iter)
    mean_ang_vel += Vec3f(iter->angular_velocity[0], iter->angular_velocity[1],
...
bool ImageProcessor::initializeFirstFrame() {
  ...
  cv::goodFeaturesToTrack(img, new_pts_, processor_config.max_features_num,
                          0.01, processor_config.min_distance);
...
bool ImageProcessor::initializeFirstFeatures(
    const std::vector<ImuData>& imu_msg_buffer) {
  // Integrate gyro data to get a guess of ratation between current and previous
  // image
  integrateImuData(R_Prev2Curr, imu_msg_buffer);
  ...
  for (int i = 0; i < curr_pts.size(); ++i) {
    ...
    if (curr_pts[i].y < 0 || curr_pts[i].y > curr_img_ptr->image.rows - 1 ||
        curr_pts[i].x < 0 || curr_pts[i].x > curr_img_ptr->image.cols - 1)
      track_inliers[i] = 0;
  }
  ...
  removeUnmarkedElements(new_pts_, track_inliers, prev_pts_inImg_);
  removeUnmarkedElements(curr_pts, track_inliers, curr_pts_inImg_);

  // Return if not enough inliers
  if (prev_pts_inImg_.size() < 20) return false;
  ...
  calcOpticalFlowPyrLK(
      curr_pyramid_, prev_pyramid_, curr_pts_inImg_, prev_pts_cpy,
  ...
  for (int i = 0; i < prev_pts_cpy.size(); ++i) {
    ...
    if (dis > 1) reverse_inliers[i] = 0;
  }
  // Remove outliers
  vector<Point2f> prev_pts_inImg(0);
  vector<Point2f> curr_pts_inImg(0);
  removeUnmarkedElements(prev_pts_inImg_, reverse_inliers, prev_pts_inImg);
  removeUnmarkedElements(curr_pts_inImg_, reverse_inliers, curr_pts_inImg);
  // Return if not enough inliers
  if (prev_pts_inImg.size() < 20) return false;
  ...
  if (!prevORBDescriptor_ptr->computeDescriptors(prev_pts_inImg, levels,
                                                 prevDescriptors) ||
      !currORBDescriptor_ptr->computeDescriptors(curr_pts_inImg, levels,
                                                 currDescriptors)) {
  ...
  for (int j = 0; j < currDescriptors.rows; ++j) {
    int dis = ORBdescriptor::computeDescriptorDistance(prevDescriptors.row(j),
                                                       currDescriptors.row(j));
    vDis.push_back(dis);
  }
  ...
  removeUnmarkedElements(prev_pts_inImg, desc_inliers, prev_pts_inlier);
  removeUnmarkedElements(curr_pts_inImg, desc_inliers, curr_pts_inlier);

  // Return if not enough inliers
  if (prev_pts_inlier.size() < 20) return false;

  // Undistort inliers
  vector<Point2f> prev_unpts_inlier(prev_pts_inlier.size());
  vector<Point2f> curr_unpts_inlier(curr_pts_inlier.size());
  undistortPoints(prev_pts_inlier, cam_intrinsics, cam_distortion_model,
                  cam_distortion_coeffs, prev_unpts_inlier, cv::Matx33d::eye(),
                  cam_intrinsics);
  undistortPoints(curr_pts_inlier, cam_intrinsics, cam_distortion_model,
                  cam_distortion_coeffs, curr_unpts_inlier, cv::Matx33d::eye(),
                  cam_intrinsics);
  ...
  findFundamentalMat(prev_unpts_inlier, curr_unpts_inlier, cv::FM_RANSAC, 1.0,
                     0.99, ransac_inliers);
  ...
  removeUnmarkedElements(desc_first, ransac_inliers, prev_desc_matched);

  // Features initialized failed if less than 20 inliers are tracked
  if (curr_pts_matched.size() < 20) return false;
...
void ImageProcessor::trackFeatures() {
  ...
  predictFeatureTracking(prev_pts_, R_Prev2Curr, cam_intrinsics, curr_points);

  // Using LK optical flow to track feaures
  vector<unsigned char> track_inliers(prev_pts_.size());
  calcOpticalFlowPyrLK(
      prev_pyramid_, curr_pyramid_, prev_pts_, curr_points, track_inliers,
      noArray(), Size(processor_config.patch_size, processor_config.patch_size),
      processor_config.pyramid_levels,
      TermCriteria(TermCriteria::COUNT + TermCriteria::EPS,
                   processor_config.max_iteration,
  ...
  for (int i = 0; i < curr_points.size(); ++i) {
    ...
    if (curr_points[i].y < 0 ||
        ...
        curr_points[i].x < 0 || curr_points[i].x > curr_img_ptr->image.cols - 1)
      track_inliers[i] = 0;
  }
  ...
  removeUnmarkedElements(pts_ids_, track_inliers, prev_inImg_ids_);
  removeUnmarkedElements(pts_lifetime_, track_inliers, prev_inImg_lifetime_);
  removeUnmarkedElements(prev_pts_, track_inliers, prev_inImg_points_);
  removeUnmarkedElements(curr_points, track_inliers, curr_inImg_points_);
  removeUnmarkedElements(init_pts_, track_inliers, init_inImg_position_);
  removeUnmarkedElements(vOrbDescriptors, track_inliers, prev_imImg_desc_);
  ...
  calcOpticalFlowPyrLK(
      curr_pyramid_, prev_pyramid_, curr_inImg_points_, prev_pts_cpy,
      reverse_inliers, noArray(),
      Size(processor_config.patch_size, processor_config.patch_size),
      processor_config.pyramid_levels,
      TermCriteria(TermCriteria::COUNT + TermCriteria::EPS,
  ...
  removeUnmarkedElements(prev_inImg_ids_, reverse_inliers, prev_inImg_ids);
  removeUnmarkedElements(prev_inImg_lifetime_, reverse_inliers,
                         prev_inImg_lifetime);
  removeUnmarkedElements(prev_inImg_points_, reverse_inliers,
                         prev_inImg_points);
  removeUnmarkedElements(curr_inImg_points_, reverse_inliers,
                         curr_inImg_points);
  removeUnmarkedElements(init_inImg_position_, reverse_inliers,
                         init_inImg_position);
  removeUnmarkedElements(prev_imImg_desc_, reverse_inliers, prev_imImg_desc);
  // Number of features left after tracking.
  ...
  Mat prevDescriptors, currDescriptors;
  if (!currORBDescriptor_ptr->computeDescriptors(curr_inImg_points, levels,
                                                 currDescriptors)) {
    cerr << "error happen while compute descriptors" << endl;
    vector<Point2f>().swap(prev_pts_);
    vector<Point2f>().swap(curr_pts_);
  ...
  removeUnmarkedElements(prev_inImg_ids, desc_inliers, prev_tracked_ids);
  removeUnmarkedElements(prev_inImg_lifetime, desc_inliers,
                         prev_tracked_lifetime);
  removeUnmarkedElements(prev_inImg_points, desc_inliers, prev_tracked_points);
  removeUnmarkedElements(curr_inImg_points, desc_inliers, curr_tracked_points);
  removeUnmarkedElements(init_inImg_position, desc_inliers,
                         init_tracked_position);
  removeUnmarkedElements(prev_imImg_desc, desc_inliers, prev_tracked_desc);
  ...
  undistortPoints(prev_tracked_points, cam_intrinsics, cam_distortion_model,
                  cam_distortion_coeffs, prev_tracked_unpts, cv::Matx33d::eye(),
                  cam_intrinsics);
  undistortPoints(curr_tracked_points, cam_intrinsics, cam_distortion_model,
                  cam_distortion_coeffs, curr_tracked_unpts, cv::Matx33d::eye(),
                  cam_intrinsics);
  ...
  //         K, cv::RANSAC, 0.999, 1.0, ransac_inliers);
  findFundamentalMat(prev_tracked_unpts, curr_tracked_unpts, cv::FM_RANSAC, 1.0,
                     0.99, ransac_inliers);
  ...
  removeUnmarkedElements(prev_tracked_ids, ransac_inliers, prev_matched_ids);
  removeUnmarkedElements(prev_tracked_lifetime, ransac_inliers,
                         prev_matched_lifetime);
  removeUnmarkedElements(prev_tracked_points, ransac_inliers,
                         prev_matched_points);
  removeUnmarkedElements(curr_tracked_points, ransac_inliers,
                         curr_matched_points);
  removeUnmarkedElements(init_tracked_position, ransac_inliers,
                         init_matched_position);
  removeUnmarkedElements(prev_tracked_desc, ransac_inliers, prev_matched_desc);
...
void ImageProcessor::trackNewFeatures() {
  ...
  removeUnmarkedElements(prev_pts_inlier, ransac_inliers, prev_pts_matched);
  removeUnmarkedElements(curr_pts_inlier, ransac_inliers, curr_pts_matched);
  removeUnmarkedElements(desc_new, ransac_inliers, prev_desc_matched);
...
void ImageProcessor::findNewFeaturesToBeTracked() {
  ...
  for (const auto& pt : curr_pts_) {
    ...
    cv::Mat mROI(mask, cv::Rect(startCol, startRow, endCol - startCol + 1,
                                endRow - startRow + 1));
    mROI.setTo(0);
  }
  ...
  if (processor_config.max_features_num - curr_pts_.size() > 0)
    cv::goodFeaturesToTrack(
        curr_img, new_pts_,
        processor_config.max_features_num - curr_pts_.size(), 0.01,
        processor_config.min_distance, mask);
}
...
void ImageProcessor::getFeatureMsg(MonoCameraMeasurementPtr feature_msg_ptr) {
  ...
  undistortPoints(curr_pts_, cam_intrinsics, cam_distortion_model,
                  cam_distortion_coeffs, curr_points_undistorted);
  undistortPoints(init_pts_, cam_intrinsics, cam_distortion_model,
                  cam_distortion_coeffs, init_points_undistorted);
  undistortPoints(prev_pts_, cam_intrinsics, cam_distortion_model,
    ...
    if (init_pts_[i].x == -1 && init_pts_[i].y == -1) {
      ...
      if (prev_is_last) {
        feature_msg_ptr->features[i].u_init_vel =
            (curr_points_undistorted[i].x - init_points_undistorted[i].x) /
            dt_2;
        feature_msg_ptr->features[i].v_init_vel =
            (curr_points_undistorted[i].y - init_points_undistorted[i].y) /
            dt_2;
      } else {
        feature_msg_ptr->features[i].u_init_vel =
            (prev_points_undistorted[i].x - init_points_undistorted[i].x) /
            dt_2;
        feature_msg_ptr->features[i].v_init_vel =
            (prev_points_undistorted[i].y - init_points_undistorted[i].y) /
            dt_2;
      }
    }
  }
