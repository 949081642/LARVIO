...
LarVio::LarVio(std::string& config_file_)
    : is_gravity_set(false), is_first_img(true), config_file(config_file_) {
  return;
}
...
bool LarVio::loadParameters() {
  ...
  Feature::optimization_config.translation_threshold =
      fsSettings["feature_translation_threshold"];
  ...
  estimate_extrin =
      (static_cast<int>(fsSettings["estimate_extrin"]) ? true : false);
  ...
  double orientation_cov, position_cov, velocity_cov, gyro_bias_cov,
      acc_bias_cov;
  ...
  // Imu instrinsics
  calib_imu =
      (static_cast<int>(fsSettings["calib_imu_instrinsic"]) ? true : false);
  ...
  state_server.M1 << state_server.Ma(1, 0), state_server.Ma(2, 0),
      state_server.Ma(2, 1);
  state_server.M2 << state_server.Ma(0, 0), state_server.Ma(1, 1),
      state_server.Ma(2, 2);
  state_server.T1 << state_server.Tg(1, 0), state_server.Tg(2, 0),
      state_server.Tg(2, 1);
  state_server.T2 << state_server.Tg(0, 0), state_server.Tg(1, 1),
      state_server.Tg(2, 2);
  state_server.T3 << state_server.Tg(0, 1), state_server.Tg(0, 2),
      state_server.Tg(1, 2);
  state_server.A1 << state_server.As(1, 0), state_server.As(2, 0),
      state_server.As(2, 1);
  state_server.A2 << state_server.As(0, 0), state_server.As(1, 1),
      state_server.As(2, 2);
  state_server.A3 << state_server.As(0, 1), state_server.As(0, 2),
      state_server.As(1, 2);
  ...
  for (int i = 0; i < 3; ++i) state_server.state_cov(i, i) = orientation_cov;
  for (int i = 3; i < 6; ++i) state_server.state_cov(i, i) = velocity_cov;
  for (int i = 6; i < 9; ++i) state_server.state_cov(i, i) = position_cov;
  for (int i = 9; i < 12; ++i) state_server.state_cov(i, i) = gyro_bias_cov;
  for (int i = 12; i < 15; ++i) state_server.state_cov(i, i) = acc_bias_cov;
  if (estimate_extrin) {
    for (int i = 15; i < 18; ++i)
  ...
  state_server.imu_state.t_cam0_imu =
      T_cam0_imu.translation() + 0.0 * Vector3d::Random();
  ...
  if_ZUPT_valid =
      (static_cast<int>(fsSettings["if_ZUPT_valid"]) ? true : false);
  // Maximum feature distance for zero velocity detection
  ...
  if (feature_idp_dim != 1 && feature_idp_dim != 3) {
  ...
  if (0 == max_features * grid_rows * grid_cols)
    ...
    cout << "Hybrid MSCKF...Maximum number of feature in state is "
         << max_features * grid_rows * grid_cols << endl;
    ...
    if (use_schmidt) cout << "Applying Schmidt EKF" << endl;
  }
  cout << "===========================================" << endl << endl;
...
bool LarVio::initialize() {
  ...
  fTakeOffStamp.open((output_dir + "msckf_2_takeoff.txt").c_str(),
                     ofstream::trunc);
  ...
  flexInitPtr.reset(new FlexibleInitializer(
      zupt_max_feature_dis, Static_Num, state_server.td, state_server.Ma,
      state_server.Tg, state_server.As, sqrt(imu_acc_noise),
      sqrt(imu_acc_bias_noise), sqrt(imu_gyro_noise), sqrt(imu_gyro_bias_noise),
...
bool LarVio::processFeatures(MonoCameraMeasurementPtr msg,
  ...
  if (!bFirstFeatures) {
    if ((imu_msg_buffer.begin() != imu_msg_buffer.end()) &&
        (imu_msg_buffer.begin()->timeStampToSec - msg->timeStampToSec -
             state_server.td <=
         0.0))
      bFirstFeatures = true;
    else
  ...
  if (!is_gravity_set) {
    if (flexInitPtr->tryIncInit(imu_msg_buffer, msg, m_gyro_old, m_acc_old,
                                state_server.imu_state)) {
      is_gravity_set = true;
      // Set take off time
  ...
  if (if_ZUPT_valid) if_ZUPT = checkZUPT();
  ...
  fImuState << state_server.imu_state.time - take_off_stamp << " " << qw << " "
            << qx << " " << qy << " " << qz << " " << vx << " " << vy << " "
            << vz << " " << px << " " << py << " " << pz << " " << bgx << " "
            << bgy << " " << bgz << " " << bax << " " << bay << " " << baz
            << " " << qbcw << " " << qbcx << " " << qbcy << " " << qbcz << " "
...
void LarVio::processModel(const double& time, const Vector3d& m_gyro,
  ...
  MatrixXd Q = Phi * G * state_server.continuous_noise_cov * G.transpose() *
               Phi.transpose() * dtime;
  state_server.state_cov.block(0, 0, LEG_DIM, LEG_DIM) =
      Phi * state_server.state_cov.block(0, 0, LEG_DIM, LEG_DIM) *
          Phi.transpose() +
      Q;

  if (state_server.state_cov.cols() > LEG_DIM) {
    state_server.state_cov.block(0, LEG_DIM, LEG_DIM,
                                 state_server.state_cov.cols() - LEG_DIM) =
...
void LarVio::predictNewState(const double& dt, const Vector3d& gyro,
                             const Vector3d& acc) {
  ...
  Matrix3d dR_dt_transpose =
      Quaterniond(dq_dt(3), dq_dt(0), dq_dt(1), dq_dt(2)).toRotationMatrix();
  Matrix3d dR_dt2_transpose =
      Quaterniond(dq_dt2(3), dq_dt2(0), dq_dt2(1), dq_dt2(2))
          .toRotationMatrix();

  // formula: k1 = f(tn, yn)
  Vector3d k1_v_dot =
      Quaterniond(q(3), q(0), q(1), q(2)).toRotationMatrix() * acc +
      IMUState::gravity;
  Vector3d k1_p_dot = v;
...
//           QXC：四元数的毕卡法更新，参见秦永元《惯性导航》第一版P301式(9.2.52)，注意角增量的计算为gyro_norm*dt
//         dq_dt2 = (cos(gyro_norm*dt*0.25)*Matrix4d::Identity() +
...
//       Quaterniond(dq_dt(3),dq_dt(0),dq_dt(1),dq_dt(2)).toRotationMatrix();
//       Matrix3d dR_dt2_transpose =
//       Quaterniond(dq_dt2(3),dq_dt2(0),dq_dt2(1),dq_dt2(2)).toRotationMatrix();

//       Vector3d acc_mid = (acc_old+acc)/2;

//       // Factor of v
//       Vector3d k1_v_dot =
//       Quaterniond(q(3),q(0),q(1),q(2)).toRotationMatrix()*acc_old +
//         IMUState::gravity;
...
//       // Update qvp
//       q = dq_dt;				//
//       QXC：所谓的4阶LK法只针对速度和位置使用，并未针对姿态做，姿态只用了前面的毕卡法更新
//       quaternionNormalize(q);	// QXC：来自本工程中的math_utils.hpp
//       v = v + dt/6*(k1_v_dot+2*k2_v_dot+2*k3_v_dot+k4_v_dot);
//       Vector3d k4_p_dot = v;
//       p = p + dt/6*(k1_p_dot+2*k2_p_dot+2*k3_p_dot+k4_p_dot);
...
void LarVio::stateAugmentation() {
  ...
  Vector3d t_c_w = state_server.imu_state.position + R_b2w * t_c_b;
  ...
  // Size of the covariance matrix without augmented features and nuisance
  // states
  ...
  state_server.state_cov.block(0, pose_cols + 6, old_rows + 6, rest_cols) =
      state_server.state_cov.block(0, pose_cols, old_rows + 6, rest_cols)
          .eval();
  ...
  state_server.state_cov.block(pose_rows + 6, pose_cols, rest_rows, 6) =
      P12_2.transpose();
...
void LarVio::addFeatureObservations(MonoCameraMeasurementPtr msg) {
  ...
  for (const auto& feature : msg->features) {
    if (map_server.find(feature.id) == map_server.end()) {
      ...
      map_server[feature.id].observations[state_id] = Vector2d(
          feature.u + feature.u_vel * dt, feature.v + feature.v_vel * dt);
      ...
      if (!(feature.u_init == -1 && feature.v_init == -1) &&
          state_server.imu_states_augment.find(state_id - 1) !=
        ...
        map_server[feature.id].observations[state_id - 1] =
            Vector2d(feature.u_init + feature.u_init_vel * dt_,
                     feature.v_init + feature.v_init_vel * dt_);
      ...
      if (if_ZUPT_valid &&
          map_server[feature.id].observations.find(state_id - 1) !=
              map_server[feature.id].observations.end()) {
  ...
  tracking_rate = static_cast<double>(tracked_feature_num) /
                  static_cast<double>(curr_feature_num);

  return;
}

void LarVio::measurementJacobian_msckf(const StateIDType& state_id,
                                       const FeatureIDType& feature_id,
                                       Matrix<double, 2, 6>& H_x,
                                       Matrix<double, 2, 6>& H_e,
  ...
  Matrix3d R_b2w =
      Quaterniond(q_imu(3), q_imu(0), q_imu(1), q_imu(2)).toRotationMatrix();
  Matrix3d R_w2b = R_b2w.transpose();
  ...
  Vector3d p_bf_w =
      (if_FEJ
           ? p_w - imu_state.position_FEJ
           : p_w - t_b_w);  // QXC: FEJ in measurement Jacobian is applied here
  ...
  dpc_dxe.leftCols(3) =
      (R_w2c * skewSymmetric(p_bf_w) * R_b2w) - (R_b2c * skewSymmetric(t_c_b));
...
void LarVio::featureJacobian_msckf(const FeatureIDType& feature_id,
  ...
  for (const auto& state_id : state_ids) {
    if (feature.observations.find(state_id) == feature.observations.end())
      continue;
  ...
  MatrixXd H_xj =
      MatrixXd::Zero(jacobian_row_size, state_server.state_cov.cols());
  ...
  for (const auto& state_id : valid_state_ids) {
    ...
    measurementJacobian_msckf(state_id, feature.id, H_xi, H_ei, H_fi, r_i);

    auto state_iter = state_server.imu_states_augment.find(state_id);
    int state_cntr =
        std::distance(state_server.imu_states_augment.begin(), state_iter);

    // Stack the Jacobians.
    H_xj.block<2, 6>(stack_cntr, LEG_DIM + 6 * state_cntr) = H_xi;
    H_xj.block<2, 6>(stack_cntr, 15) = H_ei;
    if (estimate_td)
      H_xj.block<2, 1>(stack_cntr, 21) =
          map_server[feature.id].observations_vel[state_id];
...
void LarVio::measurementJacobian_ekf_3didp(const StateIDType& state_id,
  ...
  if (state_server.imu_states_augment.find(feature.id_anchor) !=
    ...
    anch_is_nui = true;
    if (!use_schmidt) printf("shouldn't enter here if not using schmidt !!!!!");
  }
  ...
  Matrix3d R_bk2w = Quaterniond(q_imu_k(3), q_imu_k(0), q_imu_k(1), q_imu_k(2))
                        .toRotationMatrix();
  ...
  Matrix3d R_ba2w = Quaterniond(q_imu_a(3), q_imu_a(0), q_imu_a(1), q_imu_a(2))
                        .toRotationMatrix();
  ...
  if (anch_is_nui) {
    ...
    Matrix3d R_ca2w =
        Quaterniond(q_cam_a(3), q_cam_a(0), q_cam_a(1), q_cam_a(2))
            .toRotationMatrix();
  ...
  if (if_FEJ && !anch_is_nui) {
    p_ca = R_b2c *
           (R_w2ba * (feature.position_FEJ - imu_state_a.position_FEJ) - t_c_b);
  } else {
  ...
  Vector3d p_baf_w = ((if_FEJ && !anch_is_nui)
                          ? feature.position_FEJ - imu_state_a.position_FEJ
                          : p_w - t_ba_w);
  Vector3d p_bkf_w =
      (if_FEJ ? feature.position_FEJ - imu_state_k.position_FEJ : p_w - t_bk_w);
  ...
  Matrix3d Mx =
      R_w2bk * R_w2ba.transpose() * skewSymmetric(R_b2c.transpose() * p_ca);
  J_e.leftCols(3) = R_b2c * (SkewMx - Mx);
  J_e.rightCols(3) =
      R_b2c * (R_w2bk * R_w2ba.transpose() - Matrix3d::Identity());
...
void LarVio::measurementJacobian_ekf_1didp(const StateIDType& state_id,
  ...
  if (state_id == feature.id_anchor) {
    printf(
        "Measurement of anchor frame should not be used in 1d idp "
        "measurementJacobian!");
    r = Vector2d::Zero();
...
void LarVio::featureJacobian_ekf_new(const FeatureIDType& feature_id,
  ...
  for (const auto& state_id : state_ids) {
    ...
    if (1 == feature_idp_dim && state_id == feature.id_anchor)
      continue;  // Measurement of anchor frame should not be ussed when
                 // applying 1d idp
  ...
  int anchor_cntr =
      std::distance(state_server.imu_states_augment.begin(), anchor_iter);
  ...
  int feature_cntr =
      std::distance(state_server.feature_states.begin(), feature_iter);
  int feature_index;
  if (use_schmidt) {
    ...
    feature_index = state_server.state_cov.rows() +
                    feature_idp_dim * (feature_cntr - num_old);
  } else
    feature_index = LEG_DIM + 6 * state_server.imu_states_augment.size() +
                    feature_idp_dim * feature_cntr;
  ...
  for (const auto& state_id : valid_state_ids) {
    ...
    if (3 == feature_idp_dim) {
      Matrix<double, 2, 3> H_fi = Matrix<double, 2, 3>::Zero();
      measurementJacobian_ekf_3didp(state_id, feature_id, H_fi, H_ai, H_xi,
                                    H_ei, r_i);
      ...
      measurementJacobian_ekf_1didp(state_id, feature_id, H_fi, H_ai, H_xi,
                                    H_ei, r_i);
...
void LarVio::featureJacobian_ekf(const FeatureIDType& feature_id,
  ...
  if (1 == feature_idp_dim && imu_id == feature.id_anchor)
    printf(
        "Measurement of anchor frame should not appear in featureJacobian_ekf");
  ...
  int num_new =
      ...
      feature_idp_dim;
  if (use_schmidt &&
      find(state_server.nui_ids.begin(), state_server.nui_ids.end(),
           feature.id_anchor) != state_server.nui_ids.end()) {
    auto anchor_iter = find(state_server.nui_ids.begin(),
                            state_server.nui_ids.end(), feature.id_anchor);
    anchor_cntr = std::distance(state_server.nui_ids.begin(), anchor_iter);
    anchor_index =
        LEG_DIM + 6 * state_server.imu_states_augment.size() +
        feature_idp_dim * (state_server.feature_states.size() - num_new) +
        6 * anchor_cntr;
  } else if (state_server.imu_states_augment.find(feature.id_anchor) !=
             state_server.imu_states_augment.end()) {
    auto anchor_iter = state_server.imu_states_augment.find(feature.id_anchor);
    anchor_cntr =
        std::distance(state_server.imu_states_augment.begin(), anchor_iter);
  ...
  int feature_cntr =
      std::distance(state_server.feature_states.begin(), feature_iter);
  int feature_index = LEG_DIM + 6 * state_server.imu_states_augment.size() +
                      feature_idp_dim * feature_cntr;
  ...
  if (3 == feature_idp_dim) {
    Matrix<double, 2, 3> H_fi = Matrix<double, 2, 3>::Zero();
    measurementJacobian_ekf_3didp(imu_id, feature_id, H_fi, H_ai, H_xi, H_ei,
                                  r_i);

    H_x.block<2, 3>(0, feature_index) = H_fi;
  } else {
    Matrix<double, 2, 1> H_fi = Matrix<double, 2, 1>::Zero();
    measurementJacobian_ekf_1didp(imu_id, feature_id, H_fi, H_ai, H_xi, H_ei,
                                  r_i);
  ...
  if (estimate_td) H_x.block<2, 1>(0, 21) = feature.observations_vel[imu_id];
  r = r_i;

  return;
}

void LarVio::measurementUpdate_msckf(const MatrixXd& H, const VectorXd& r) {
  ...
  if (H.rows() > H.cols()) {
    ...
    H_thin =
        H_temp.topRows(LEG_DIM + state_server.imu_states_augment.size() * 6);
  ...
  const Vector4d dq_extrinsic =
      smallAngleQuaternion(delta_x_legacy.segment<3>(15));
  state_server.imu_state.R_imu_cam0 =
      state_server.imu_state.R_imu_cam0 *
      Quaterniond(dq_extrinsic(3), dq_extrinsic(0), dq_extrinsic(1),
                  dq_extrinsic(2))
          .toRotationMatrix()
          .transpose();
  ...
  for (int i = 0; i < state_server.imu_states_augment.size();
    ...
    Quaterniond Q_IMU =
        Quaterniond(dq_IMU(3), dq_IMU(0), dq_IMU(1), dq_IMU(2)) *
        Quaterniond(q_IMU(3), q_IMU(0), q_IMU(1), q_IMU(2));
    ...
    Matrix3d R_c2b = state_server.imu_state.R_imu_cam0.transpose();
    const Vector3d& t_c_b = state_server.imu_state.t_cam0_imu;
  ...
  for (int i = 0; i < state_server.feature_states.size(); ++i, feature_itr++) {
    ...
    if (use_schmidt &&
      ...
      imu_state_aug =
          state_server.nui_imu_states[map_server[feature_id].id_anchor];
    else if (state_server.imu_states_augment.find(
                 map_server[feature_id].id_anchor) !=
             state_server.imu_states_augment.end())
      imu_state_aug =
          state_server.imu_states_augment[map_server[feature_id].id_anchor];
    else
      printf("ERROR HAPPENED IN updt_msckf");
    const Vector4d& cam_qua = imu_state_aug.orientation_cam;
    Matrix3d R_c2w = Quaterniond(cam_qua(3), cam_qua(0), cam_qua(1), cam_qua(2))
                         .toRotationMatrix();
    ...
    if (3 == feature_idp_dim) {
      ...
      p_c(0) = map_server[feature_id].invParam(0) /
               map_server[feature_id].invParam(2);
      p_c(1) = map_server[feature_id].invParam(1) /
               map_server[feature_id].invParam(2);
      ...
      // Update position in world frame
      p_c(0) = map_server[feature_id].obs_anchor(0) /
               map_server[feature_id].invDepth;
      p_c(1) = map_server[feature_id].obs_anchor(1) /
               map_server[feature_id].invDepth;
  ...
  if (use_schmidt && state_server.nui_ids.size() > 0) {
    ...
    P = I_KH * P;
    P.block(base_cntr + feature_idp_dim * state_server.feature_states.size(),
            base_cntr + feature_idp_dim * state_server.feature_states.size(),
            6 * state_server.nui_ids.size(), 6 * state_server.nui_ids.size()) =
        P_nui;
  } else {
...
void LarVio::measurementUpdate_hybrid(const Eigen::MatrixXd& H_ekf_new,
                                      const Eigen::VectorXd& r_ekf_new,
                                      const Eigen::MatrixXd& H_ekf,
                                      const Eigen::VectorXd& r_ekf,
                                      const Eigen::MatrixXd& H_msckf,
                                      const Eigen::VectorXd& r_msckf) {
  ...
  H_o.block(0, 0, H_msckf.rows(), state_server.state_cov.cols()) = H_msckf;
  H_o.block(H_msckf.rows(), 0, H_ekf.rows(), state_server.state_cov.cols()) =
      H_ekf;
  H_o.block(H_msckf.rows() + H_ekf.rows(), 0, H_ekf_new.rows() - sz_new,
            state_server.state_cov.cols()) =
      H_ekf_new.block(0, 0, H_ekf_new.rows() - sz_new,
                      state_server.state_cov.cols());
  // H1
  MatrixXd H_1 = H_ekf_new.block(H_ekf_new.rows() - sz_new, 0, sz_new,
                                 state_server.state_cov.cols());
  // H2
  MatrixXd H_2 = H_ekf_new.block(H_ekf_new.rows() - sz_new,
                                 state_server.state_cov.cols(), sz_new, sz_new);
  ...
  r_o.segment(r_msckf.rows() + r_ekf.rows(), r_ekf_new.rows() - sz_new) =
  ...
  MatrixXd S =
      H_o * P * H_o.transpose() +
      feature_observation_noise * MatrixXd::Identity(H_o.rows(), H_o.rows());
  ...
  for (int i = 0; i < state_server.feature_states.size(); ++i, feature_itr++) {
    ...
    if (3 == feature_idp_dim) {
      ...
      if (use_schmidt && i >= num_old)
        delta_f_aug = delta_x.segment<3>(state_server.state_cov.rows() +
                                         (i - num_old) * 3);
      else
  ...
  if (use_schmidt && state_server.nui_ids.size() > 0) {
    MatrixXd P_nui = P.block(base_cntr + feature_idp_dim * num_old,
  ...
  if (r_1.rows() > 0) {
    ...
    MatrixXd P22 = -nHHP * HH.transpose() +
                   feature_observation_noise *
                       (H22.ldlt().solve(MatrixXd::Identity(sz_new, sz_new)));
    ...
    if (use_schmidt && state_server.nui_ids.size() > 0) {
      ...
      // Fill in new blocks
      P.block(old_rows - nui_rows, 0, sz_new, old_cols - nui_cols) =
          nHHP.leftCols(old_cols - nui_cols);
      P.block(old_rows - nui_rows, old_cols + sz_new - nui_cols, sz_new,
              nui_cols) = nHHP.rightCols(nui_cols);
      P.block(0, old_cols - nui_cols, old_rows - nui_rows, sz_new) =
          nHHP.leftCols(old_cols - nui_cols).transpose();
      P.block(old_rows + sz_new - nui_rows, old_cols - nui_cols, nui_rows,
              sz_new) = nHHP.rightCols(nui_cols).transpose();
...
bool LarVio::gatingTest(const MatrixXd& H, const VectorXd& r, const int& dof) {
  MatrixXd P1 = H * state_server.state_cov * H.transpose();
  MatrixXd P2 =
      feature_observation_noise * MatrixXd::Identity(H.rows(), H.rows());
...
void LarVio::removeLostFeatures() {
  ...
  for (auto iter = map_server.begin(); iter != map_server.end(); ++iter) {
    ...
    bool if_tracked_now =
        feature.observations.find(state_server.imu_state.id) !=
  ...
  if (use_schmidt) rmUselessNuisanceState();
    ...
    if (!feature.in_state) {
      if (!if_tracked_now) {
        ...
        if (!feature.is_initialized) {
          if (!feature.checkMotion(state_server.imu_states_augment,
                                   if_tracked_now)) {
            invalid_feature_ids.push_back(feature.id);
            continue;
          } else {
            if (!feature.initializePosition(state_server.imu_states_augment,
        ...
        // TRUE if this feature is tracked now and have been tracked for too
        // long
        ...
        // Use this feature as ekf feature or msckf feature base on the
        // occupation of map grid
        Vector2d xy =
        ...
        if (grid_map[code].size() < max_features &&
            state_server.imu_state.time - last_ZUPT_time > 5 &&
            (state_server.feature_states.size() + ekf_new_feature_ids.size()) <
                max_features * grid_rows * grid_cols) {
          // Intitialize it as ekf feature no matter if it hasn't been
          // initialized as one.
          if (!feature.ekf_feature) {
            feature.is_initialized = false;
            if (feature.checkMotion(state_server.imu_states_augment,
                                    if_tracked_now))
              feature.initializeInvParamPosition(
                  state_server.imu_states_augment, state_server.imu_state.id);
          }
          // Pass the feature that failed initialization
          if (!feature.is_initialized) continue;
          if (1 == feature_idp_dim)
            jacobian_row_size_ekf_new +=
                2 * (feature.observations.size() -
                     1);  // observation of anchor frame is not used when 1d idp
          else
          ...
          if (!feature.is_initialized) {
            if (feature.checkMotion(state_server.imu_states_augment,
                                    if_tracked_now))
              feature.initializePosition(state_server.imu_states_augment,
                                         state_server.imu_state.id);
          }
          if (!feature.is_initialized) continue;
          jacobian_row_size_msckf += 2 * feature.observations.size() - 3;
  ...
  if (msckf_feature_ids.size() == 0 && ekf_new_feature_ids.size() == 0 &&
  ...
  if (!if_ZUPT) {
    ...
    MatrixXd H_ekf_new =
        MatrixXd::Zero(jacobian_row_size_ekf_new,
                       state_server.state_cov.cols() +
    ...
    for (const auto& feature_id : ekf_new_feature_ids) {
      ...
      if (gatingTest(H_xj_, r_j_, 2 * state_ids.size() - 3)) {
        ...
        if (1 == feature_idp_dim)
          jacobian_row_size_ekf_new =
              jacobian_row_size_ekf_new - 2 * (feature.observations.size() - 1);
        else
          jacobian_row_size_ekf_new =
              jacobian_row_size_ekf_new - 2 * feature.observations.size();
      }
    ...
    for (const auto& feature_id : invalid_new_feature_ids) {
      ...
      int feature_sequence =
          std::distance(ekf_new_feature_ids.begin(), feature_itr);
      int feature_state_start =
          state_server.state_cov.cols() + feature_idp_dim * feature_sequence;
      ...
      if (feature_state_end < H_ekf_new.cols()) {
        H_ekf_new.block(0, feature_state_start, H_ekf_new.rows(),
                        H_ekf_new.cols() - feature_state_end) =
            H_ekf_new.block(0, feature_state_end, H_ekf_new.rows(),
                            H_ekf_new.cols() - feature_state_end);
      }
      H_ekf_new.conservativeResize(H_ekf_new.rows(),
                                   H_ekf_new.cols() - feature_idp_dim);
    ...
    if (ekf_new_feature_ids.size() > 0) {
      ...
      const MatrixXd& H_f_ekf_new =
          H_ekf_new.rightCols(feature_idp_dim * ekf_new_feature_ids.size());
      ...
      spqr_helper.compute(H_sparse);
      MatrixXd Q;
      MatrixXd IdenMx = MatrixXd::Identity(jacobian_row_size_ekf_new,
                                           jacobian_row_size_ekf_new);
      ...
      MatrixXd W =
          MatrixXd::Zero(jacobian_row_size_ekf_new, jacobian_row_size_ekf_new);
      W.leftCols(jacobian_row_size_ekf_new -
                 feature_idp_dim * ekf_new_feature_ids.size()) = V;
      W.rightCols(feature_idp_dim * ekf_new_feature_ids.size()) = U;
      H_ekf_new = W.transpose() * H_ekf_new;
      r_ekf_new = W.transpose() * r_ekf_new;
    } else {
      H_ekf_new =
          MatrixXd::Zero(0, state_server.state_cov.cols() +
                                feature_idp_dim * ekf_new_feature_ids.size());
    ...
    MatrixXd H_ekf =
        MatrixXd::Zero(jacobian_row_size_ekf, state_server.state_cov.cols());
    ...
    // Decompose the final Jacobian matrix to reduce computational complexity as
    // in Equation (28), (29).
    MatrixXd H_ekf_thin;
    VectorXd r_ekf_thin;
    ...
    if (H_ekf_thin.rows() < H_ekf.rows())  // debug log
      printf("H_ekf.row = %ld, H_ekf_thin.rows = %ld", H_ekf.rows(),
             H_ekf_thin.rows());
    H_ekf = MatrixXd::Zero(H_ekf_thin.rows(), state_server.state_cov.cols());
    H_ekf.leftCols(H_ekf_thin.cols()) = H_ekf_thin;
    r_ekf = r_ekf_thin;

    // Stacked Jacobian and residuals for MSCKF features
    MatrixXd H_msckf =
        MatrixXd::Zero(jacobian_row_size_msckf,
                       LEG_DIM + 6 * state_server.imu_states_augment.size());
    ...
    for (const auto& feature_id : msckf_feature_ids) {
      ...
      if (gatingTest(H_xj, r_j, 2 * state_ids.size() - 3)) {
        H_msckf.block(stack_cntr, 0, H_xj.rows(), H_msckf.cols()) =
            H_xj.leftCols(H_msckf.cols());
    ...
    if (H_msckf.rows() == 0 || r_msckf.rows() == 0 ||
      ...
      H_msckf_thin =
          H_temp.topRows(LEG_DIM + state_server.imu_states_augment.size() * 6);
      r_msckf_thin =
          r_temp.head(LEG_DIM + state_server.imu_states_augment.size() * 6);
    }
    H_msckf =
        MatrixXd::Zero(H_msckf_thin.rows(), state_server.state_cov.cols());
    H_msckf.leftCols(H_msckf_thin.cols()) = H_msckf_thin;
    r_msckf = r_msckf_thin;

    // Measurement update.
    measurementUpdate_hybrid(H_ekf_new, r_ekf_new, H_ekf, r_ekf, H_msckf,
                             r_msckf);
...
void LarVio::findRedundantImuStates(vector<StateIDType>& rm_state_ids) {
  // Move the iterator to the key position.
  auto key_state_iter = state_server.imu_states_augment.end();
  for (int i = 0; i < 4; ++i) --key_state_iter;
  auto state_iter = key_state_iter;
  ++state_iter;
  auto first_state_iter = state_server.imu_states_augment.begin();

  // Pose of the key camera state.
  const Vector3d key_position = key_state_iter->second.position_cam;
  const Vector4d& key_qua = key_state_iter->second.orientation_cam;
  const Matrix3d key_rotation =
      Quaterniond(key_qua(3), key_qua(0), key_qua(1), key_qua(2))
          .toRotationMatrix();

  // Mark the camera states to be removed based on the
  // motion between states.
  for (int i = 0; i < 2; ++i) {
    const Vector3d position = state_iter->second.position_cam;
    const Vector4d& qua = state_iter->second.orientation_cam;
    const Matrix3d rotation = Quaterniond(qua(3), qua(0), qua(1), qua(2))
                                  .toRotationMatrix()
                                  .transpose();

    double distance = (position - key_position).norm();
    double angle = AngleAxisd(rotation * key_rotation).angle();
...
void LarVio::pruneImuStateBuffer() {
  ...
  if (!if_ZUPT) {
    if (state_server.imu_states_augment.size() < sw_size) return;
  ...
  for (auto& item : map_server) {
    ...
    if (feature.in_state) {
      ...
      vector<StateIDType>::iterator it =
          find(involved_state_ids.begin(), involved_state_ids.end(),
               feature.id_anchor);
      if (it != involved_state_ids.end()) {
        // if (use_schmidt) {
        if (use_schmidt &&
            state_server.imu_state.id - feature.id_anchor >
                2) {  // Only use mature pose as nuisance state, the threshold
                      // '2' is decide by criterion in findRedundantImuStates
          ...
          if (find(new_nui_state_ids.begin(), new_nui_state_ids.end(),
                   feature.id_anchor) == new_nui_state_ids.end())
          ...
          if (3 == feature_idp_dim) {
            ...
            // New anchor pose.
            const IMUState_Aug& imu_state_new =
                state_server.imu_states_augment[new_id];
            const Vector4d& cam_qua_new = imu_state_new.orientation_cam;
            Matrix3d R_c2w_new = Quaterniond(cam_qua_new(3), cam_qua_new(0),
                                             cam_qua_new(1), cam_qua_new(2))
                                     .toRotationMatrix();
            ...
            updateFeatureCov_3didp(feature.id, feature.id_anchor, new_id);
          } else {
            // New anchor ID.
            ...
            const Vector3d& p_w = feature.position;
            Vector3d p_new = R_c2w_new.inverse() * (p_w - t_c_w_new);
            // Compute new inverse depth and correct observation for new anchor
            // frame.
            feature.invDepth = 1 / p_new(2);
            // Fix observation if the correction is small
            Vector2d obs_fix =
                Vector2d(p_new(0) / p_new(2), p_new(1) / p_new(2));
            feature.obs_anchor(0) = obs_fix(0);
            feature.obs_anchor(1) = obs_fix(1);

            updateFeatureCov_1didp(feature.id, feature.id_anchor, new_id);
          }
      ...
      if (feature.is_initialized) {
        vector<StateIDType>::iterator it =
            find(involved_state_ids.begin(), involved_state_ids.end(),
                 feature.id_anchor);
        if (it != involved_state_ids.end()) {
          if (0) {  // NOTE: Potential EKF features does not produce nuisance
                    // state
            ...
            if (3 == feature_idp_dim) {
              ...
              const Vector3d& p_w = feature.position;
              Vector3d p_new = R_c2w_new.inverse() * (p_w - t_c_w_new);
              // Compute new inverse depth and correct observation for new
              // anchor frame.
              feature.invDepth = 1 / p_new(2);
      ...
      if (!if_ZUPT && !feature.ekf_feature && involved_state_ids.size() > 1) {
        bool if_tracked =
            feature.observations.find(state_server.imu_state.id) !=
            feature.observations.end();
        if (!feature.is_initialized) {
          ...
          if (!feature.checkMotion(state_server.imu_states_augment,
            ...
            if (!feature.initializePosition_AssignAnchor(
                    state_server.imu_states_augment)) {
              continue;
            }
          }
        }
  ...
  for (const auto& state_id : rm_imu_state_ids) {
    int state_sequence =
        std::distance(state_server.imu_states_augment.begin(),
                      state_server.imu_states_augment.find(state_id));
    int state_start = LEG_DIM + 6 * state_sequence;
    int state_end = state_start + 6;

    if (use_schmidt && find(new_nui_state_ids.begin(), new_nui_state_ids.end(),
                            state_id) != new_nui_state_ids.end()) {
      ...
      if (state_end < state_server.state_cov.rows()) {
        ...
        MatrixXd P_os_1 =
            state_server.state_cov.block(state_start, 0, 6, state_start);
        MatrixXd P_os_2 = state_server.state_cov.block(
            state_start, state_end, 6,
        ...
        state_server.state_cov.block(state_start, 0,
                                     ...
                                     state_server.state_cov.cols()) =
            state_server.state_cov.block(
                state_end, 0, state_server.state_cov.rows() - state_end,
                state_server.state_cov.cols());

        state_server.state_cov.block(
            0, state_start, state_server.state_cov.rows(),
            state_server.state_cov.cols() - state_end) =
            state_server.state_cov.block(
                0, state_end, state_server.state_cov.rows(),
                state_server.state_cov.cols() - state_end);

        state_server.state_cov.block<6, 6>(state_server.state_cov.rows() - 6,
                                           state_server.state_cov.cols() - 6) =
            P_ss;
        state_server.state_cov.block(state_server.state_cov.rows() - 6, 0, 6,
                                     state_start) = P_os_1;
        state_server.state_cov.block(
            state_server.state_cov.rows() - 6, state_start, 6,
            state_server.state_cov.cols() - state_end) = P_os_2;
        state_server.state_cov.block(0, state_server.state_cov.cols() - 6,
      ...
      state_server.nui_imu_states[state_id] =
          state_server.imu_states_augment[state_id];
    } else {
...
Eigen::Isometry3d LarVio::getTbw() {
  ...
  T_i_w.linear() = Quaterniond(imu_qua(3), imu_qua(0), imu_qua(1), imu_qua(2))
                       .toRotationMatrix();
  T_i_w.translation() = imu_state.position;

  Eigen::Isometry3d T_b_w =
      IMUState::T_imu_body * T_i_w * IMUState::T_imu_body.inverse();
...
void LarVio::getStableMapPointPositions(
    std::map<larvio::FeatureIDType, Eigen::Vector3d>& mMapPoints) {
  for (auto item : lost_slam_features)
    mMapPoints[item.first] = item.second.position;
  lost_slam_features.clear();
}

void LarVio::getActiveeMapPointPositions(
    std::map<larvio::FeatureIDType, Eigen::Vector3d>& mMapPoints) {
...
void LarVio::resetFejPoint() {
  ...
  for (auto iter = state_server.imu_states_augment.begin();
       iter != state_server.imu_states_augment.end(); ++iter) {
    iter->second.position_FEJ = iter->second.position;
  }

  // debug log
  printf("RESET FIRST ESTIMATE POINT AT %fs!",
         state_server.imu_state.time - take_off_stamp);
}
...
void LarVio::measurementUpdate_ZUPT_vpq() {
  ...
  H.block<3, 3>(3, LEG_DIM + 6 * N - 3) =
      Matrix3d::Identity();  // zupt_p current
  H.block<3, 3>(3, LEG_DIM + 6 * N - 9) =
      -Matrix3d::Identity();  // zupt_p previous
  H.block<3, 3>(6, LEG_DIM + 6 * N - 6) =
      -0.5 * Matrix3d::Identity();  // zupt_q current
  H.block<3, 3>(6, LEG_DIM + 6 * N - 12) =
      0.5 * Matrix3d::Identity();  // zupt_q previous
  ...
  const Vector3d& p_curr =
      state_server.imu_states_augment[state_server.imu_state.id].position;
  const Vector3d& p_prev =
      state_server.imu_states_augment[state_server.imu_state.id - 1].position;
  r.segment<3>(3) = -(p_curr - p_prev);
  // r_q
  const Vector4d& q_c =
      state_server.imu_states_augment[state_server.imu_state.id].orientation;
  Quaterniond q_curr(q_c(3), q_c(0), q_c(1), q_c(2));
  const Vector4d& q_p =
      state_server.imu_states_augment[state_server.imu_state.id - 1]
          .orientation;
...
void LarVio::updateFeatureCov_3didp(const FeatureIDType& feature_id,
                                    const StateIDType& old_state_id,
                                    const StateIDType& new_state_id) {
  ...
  const IMUState_Aug& imu_state_old =
      state_server.imu_states_augment[old_state_id];
  Vector4d q_imu_old = imu_state_old.orientation;
  Matrix3d R_b2w_old =
      Quaterniond(q_imu_old(3), q_imu_old(0), q_imu_old(1), q_imu_old(2))
          .toRotationMatrix();
  const Vector3d& t_b_w_old = imu_state_old.position;
  const Vector4d& cam_qua_old = imu_state_old.orientation_cam;
  Matrix3d R_c2w_old = Quaterniond(cam_qua_old(3), cam_qua_old(0),
                                   cam_qua_old(1), cam_qua_old(2))
                           .toRotationMatrix();
  ...
  Vector3d p_old;
  if (if_FEJ) {
    p_old = R_b2c * (R_b2w_old.transpose() *
                         (feature.position_FEJ - imu_state_old.position_FEJ) -
                     t_c_b);
  } else
  ...
  Matrix3d R_b2w_new =
      Quaterniond(q_imu_new(3), q_imu_new(0), q_imu_new(1), q_imu_new(2))
          .toRotationMatrix();
  ...
  Matrix3d Mx =
      R_w2b_new * R_b2w_old * skewSymmetric(R_b2c.transpose() * p_old);
  J_e.leftCols(3) = R_b2c * (SkewMx - Mx);
  ...
  int old_state_cntr =
      std::distance(state_server.imu_states_augment.begin(), old_state_iter);
  auto new_state_iter = state_server.imu_states_augment.find(old_state_id);
  int new_state_cntr =
      std::distance(state_server.imu_states_augment.begin(), new_state_iter);
  auto feature_iter = find(state_server.feature_states.begin(),
                           state_server.feature_states.end(), feature_id);
  int feature_cntr =
      std::distance(state_server.feature_states.begin(), feature_iter);
  ...
  MatrixXd Pfleg_left =
      Pfleg.block(0, 0, 3, LEG_DIM + 6 * state_size + 3 * feature_cntr);
  MatrixXd Pfleg_right;
  if (use_schmidt) {
    Pfleg_right =
        Pfleg.block(0, LEG_DIM + 6 * state_size + 3 * feature_cntr + 3, 3,
                    3 * feature_size - 3 * feature_cntr - 3 +
                        6 * state_server.nui_ids.size());
  } else {
    Pfleg_right =
        Pfleg.block(0, LEG_DIM + 6 * state_size + 3 * feature_cntr + 3, 3,
  ...
  // Update covariance.
  state_server.state_cov.block(LEG_DIM + 6 * state_size + 3 * feature_cntr,
                               LEG_DIM + 6 * state_size + 3 * feature_cntr, 3,
                               3) = Pff;
  state_server.state_cov.block(LEG_DIM + 6 * state_size + 3 * feature_cntr, 0,
                               3, LEG_DIM + 6 * state_size + 3 * feature_cntr) =
      Pfleg_left;
  state_server.state_cov.block(0, LEG_DIM + 6 * state_size + 3 * feature_cntr,
                               LEG_DIM + 6 * state_size + 3 * feature_cntr, 3) =
      Pfleg_left.transpose();
  if (use_schmidt) {
    state_server.state_cov.block(
        LEG_DIM + 6 * state_size + 3 * feature_cntr,
        LEG_DIM + 6 * state_size + 3 * feature_cntr + 3, 3,
        3 * feature_size - 3 * feature_cntr - 3 +
            6 * state_server.nui_ids.size()) = Pfleg_right;
    state_server.state_cov.block(
        LEG_DIM + 6 * state_size + 3 * feature_cntr + 3,
        LEG_DIM + 6 * state_size + 3 * feature_cntr,
        3 * feature_size - 3 * feature_cntr - 3 +
            6 * state_server.nui_ids.size(),
        3) = Pfleg_right.transpose();
  } else {
    state_server.state_cov.block(
        LEG_DIM + 6 * state_size + 3 * feature_cntr,
        LEG_DIM + 6 * state_size + 3 * feature_cntr + 3, 3,
        3 * feature_size - 3 * feature_cntr - 3) = Pfleg_right;
    state_server.state_cov.block(
        LEG_DIM + 6 * state_size + 3 * feature_cntr + 3,
        LEG_DIM + 6 * state_size + 3 * feature_cntr,
...
void LarVio::updateFeatureCov_1didp(const FeatureIDType& feature_id,
  ...
  Vector3d f_old = Vector3d(p_old_(0) / p_old_(2), p_old_(1) / p_old_(2), 1);

  // Poses of new camera and corresponding imu.
  const IMUState_Aug& imu_state_new =
      state_server.imu_states_augment[new_state_id];
  Vector4d q_imu_new = imu_state_new.orientation;
  Matrix3d R_b2w_new =
      Quaterniond(q_imu_new(3), q_imu_new(0), q_imu_new(1), q_imu_new(2))
          .toRotationMatrix();
  ...
  MatrixXd Pfleg_left =
      Pfleg.block(0, 0, 1, LEG_DIM + 6 * state_size + feature_cntr);
  MatrixXd Pfleg_right;
  if (use_schmidt) {
    Pfleg_right = Pfleg.block(
        0, LEG_DIM + 6 * state_size + feature_cntr + 1, 1,
        feature_size - feature_cntr - 1 + 6 * state_server.nui_ids.size());
  } else {
    Pfleg_right = Pfleg.block(0, LEG_DIM + 6 * state_size + feature_cntr + 1, 1,
                              feature_size - feature_cntr - 1);
  }

  // Update covariance.
  state_server.state_cov.block(LEG_DIM + 6 * state_size + feature_cntr,
                               LEG_DIM + 6 * state_size + feature_cntr, 1, 1) =
      Pff;
  state_server.state_cov.block(LEG_DIM + 6 * state_size + feature_cntr, 0, 1,
                               LEG_DIM + 6 * state_size + feature_cntr) =
      Pfleg_left;
  state_server.state_cov.block(0, LEG_DIM + 6 * state_size + feature_cntr,
  ...
  if (use_schmidt) {
    ...
    state_server.state_cov.block(
        LEG_DIM + 6 * state_size + feature_cntr + 1,
        LEG_DIM + 6 * state_size + feature_cntr,
        feature_size - feature_cntr - 1 + 6 * state_server.nui_ids.size(), 1) =
        Pfleg_right.transpose();
  } else {
    state_server.state_cov.block(LEG_DIM + 6 * state_size + feature_cntr,
                                 LEG_DIM + 6 * state_size + feature_cntr + 1, 1,
                                 feature_size - feature_cntr - 1) = Pfleg_right;
    state_server.state_cov.block(LEG_DIM + 6 * state_size + feature_cntr + 1,
                                 LEG_DIM + 6 * state_size + feature_cntr,
                                 feature_size - feature_cntr - 1, 1) =
        Pfleg_right.transpose();
  }
...
void LarVio::rmLostFeaturesCov(const std::vector<FeatureIDType>& lost_ids) {
  ...
  for (const auto& feature_id : lost_ids) {
    ...
    auto feature_itr = find(state_server.feature_states.begin(),
                            state_server.feature_states.end(), feature_id);
    int feature_sequence =
        std::distance(state_server.feature_states.begin(), feature_itr);
    int feature_state_start = LEG_DIM +
                              6 * state_server.imu_states_augment.size() +
                              feature_idp_dim * feature_sequence;
    int feature_state_end = feature_state_start + feature_idp_dim;
    ...
    if (feature_state_end < state_server.state_cov.rows()) {
      ...
      state_server.state_cov.block(
          0, feature_state_start, state_server.state_cov.rows(),
          state_server.state_cov.cols() - feature_state_end) =
          state_server.state_cov.block(
              0, feature_state_end, state_server.state_cov.rows(),
              state_server.state_cov.cols() - feature_state_end);
    }
    state_server.state_cov.conservativeResize(
        state_server.state_cov.rows() - feature_idp_dim,
        state_server.state_cov.cols() - feature_idp_dim);

    // Remove this feature from its nuisance anchor if needed
    if (use_schmidt) {
      StateIDType id_an = map_server[feature_id].id_anchor;
      if (find(state_server.nui_ids.begin(), state_server.nui_ids.end(),
               id_an) != state_server.nui_ids.end()) {
...
void LarVio::updateGridMap() {
  if (0 == grid_rows * grid_cols) return;
...
void LarVio::delRedundantFeatures() {
  ...
  for (int i = 0; i < grid_rows * grid_cols; ++i) {
    ...
    sort(id_num.begin(), id_num.end(), LarVio::compareByObsNum);
    auto it = id_num.begin();
    for (int j = 0; j < v_IDs.size() - max_features; ++j, ++it)
...
StateIDType LarVio::getNewAnchorId(Feature& feature,
                                   const std::vector<StateIDType>& rmIDs) {
  ...
  for (int i = 0; i < size; ++i) --key_state_iter;
  ...
  for (int i = 0; i < looplen; ++i, ++key_state_iter) {
    ...
    if (bNotRm) {
      ...
      Matrix3d R_c2w =
          Quaterniond(cam_qua(3), cam_qua(0), cam_qua(1), cam_qua(2))
              .toRotationMatrix();
      const Vector3d& t_c_w = key_state_iter->second.position_cam;
      Vector3d p_new = R_c2w.inverse() * (p_w - t_c_w);
      Vector2d vdis(
          p_new(0) / p_new(2) - feature.observations[key_state_iter->first](0),
...
void LarVio::calPhi(Eigen::MatrixXd& Phi, const double& dtime,
                    ...
                    const Eigen::Vector3d& acc_old,
                    const Eigen::Vector3d& gyro_old) {
  Vector3d f_mid = (f + f_old) / 2;
  Vector3d acc_mid = (acc + acc_old) / 2;
  Vector3d w_mid = (w_old + w) / 2 + dtime * (w_old.cross(w)) / 12;

  Vector3d Axis_Angle = dtime * (gyro_old + gyro) / 2 +
                        dtime * dtime * (gyro_old.cross(gyro)) / 12;
  ...
  // Simple blocks
  Vector3d vk = (if_FEJ ? state_server.imu_state_FEJ_old.velocity
                        : state_server.imu_state_old.velocity);
  Vector3d pk = (if_FEJ ? state_server.imu_state_FEJ_old.position
                        : state_server.imu_state_old.position);
  Vector3d vkp1 = (if_FEJ ? state_server.imu_state_FEJ_now.velocity
                          : state_server.imu_state.velocity);
  Vector3d pkp1 = (if_FEJ ? state_server.imu_state_FEJ_now.position
                          : state_server.imu_state.position);
  Vector3d g_w = IMUState::gravity;
  Phi.block<3, 3>(0, 9) =  // Phi_q_bg
      -0.5 * C_bk2w * (2 * Matrix3d::Identity() + AxisAngle_hat) * dtime *
      state_server.Tg;
  Phi.block<3, 3>(0, 12) =  // Phi_q_ba
      0.5 * C_bk2w * (2 * Matrix3d::Identity() + AxisAngle_hat) * dtime * TA *
      state_server.Ma;
  Phi.block<3, 3>(3, 0) =  // Phi_v_q
      -skewSymmetric(vkp1 - vk - g_w * dtime);
  Phi.block<3, 3>(3, 9) =  // Phi_v_bg
      skewSymmetric(-pkp1 + pk + vkp1 * dtime - 0.5 * g_w * dtime * dtime) *
          C_bk2w +
      skewSymmetric(-0.5 * pkp1 + 0.5 * pk + 0.5 * vkp1 * dtime -
                    g_w * dtime * dtime / 6) *
          C_bk2w * AxisAngle_hat;
  ...
  Phi.block<3, 3>(6, 9) =  // Phi_p_bg
      ...
      dtime * skewSymmetric(pkp1 - pk - g_w * dtime * dtime / 6) * C_bk2w *
          AxisAngle_hat / 4;
  Phi.block<3, 3>(6, 12) =  // Phi_p_ba
      -C_bk2w * (3 * Matrix3d::Identity() + AxisAngle_hat) * dtime * dtime / 6 *
          state_server.Ma -
  ...
  if (calib_imu) {
    ...
    Matrix3d kvM1_2 =
        R_mid * FL_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM1_1 / 2;
    Matrix3d kvM1_3 =
        R_mid * FL_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM1_2 / 2;
    ...
    Matrix3d kvM2_2 =
        R_mid * FD_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM2_1 / 2;
    Matrix3d kvM2_3 =
        R_mid * FD_kh + skewSymmetric(R_mid * acc_mid) * dtime * kqM2_2 / 2;
...
void LarVio::rmUselessNuisanceState() {
  ...
  for (int i = 0; i < state_server.nui_ids.size(); ++i) {
    ...
    if (0 == state_server.nui_features[id_nui].size()) rm_ids.push_back(id_nui);
  }
  ...
  for (int i = 0; i < rm_ids.size(); ++i) {
    ...
    // Get index
    auto nui_itr =
        find(state_server.nui_ids.begin(), state_server.nui_ids.end(), id_nui);
    int nui_sequence = std::distance(state_server.nui_ids.begin(), nui_itr);
    int nui_state_start = LEG_DIM + 6 * state_server.imu_states_augment.size() +
                          feature_idp_dim * state_server.feature_states.size() +
                          6 * nui_sequence;
    ...
    if (nui_state_end < state_server.state_cov.cols()) {
      state_server.state_cov.block(
          nui_state_start, 0, state_server.state_cov.rows() - nui_state_end,
          state_server.state_cov.cols()) =
          state_server.state_cov.block(
              nui_state_end, 0, state_server.state_cov.rows() - nui_state_end,
              state_server.state_cov.cols());
      state_server.state_cov.block(
          0, nui_state_start, state_server.state_cov.rows(),
          state_server.state_cov.cols() - nui_state_end) =
          state_server.state_cov.block(
              0, nui_state_end, state_server.state_cov.rows(),
              state_server.state_cov.cols() - nui_state_end);
    }
    state_server.state_cov.conservativeResize(
        state_server.state_cov.rows() - 6, state_server.state_cov.cols() - 6);
